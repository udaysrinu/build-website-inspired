...
description: Research a category of premium websites, extract their design tokens, and merge them into a single coherent theme you can paste into any app. Use when the user says "build me a UI inspired by <category>", "design tokens for a <adjective> <product-type>", "make me a theme that feels like <vibe>", or names 2+ reference sites to synthesize from. Outputs tokens only (CSS variables, JSON, markdown rationale) — no build.
...
Your input is `$ARGUMENTS` — a category description, vibe, or a list of sites to synthesize from. Examples:
...
2. Read `$ARGUMENTS` as the category description. If it is already a list of URLs, skip straight to Phase 1 with those URLs as the reference set.
...
Interpret `$ARGUMENTS` and propose a shortlist of reference sites. You are curating a tasteful corpus — not Googling a generic list.
...
If `$ARGUMENTS` doesn't fuzzy-match one of these, don't force it — propose sites you know fit the vibe from first principles and state your reasoning per site.
...
Prompt: "<user's $ARGUMENTS verbatim>"
Generated: <ISO date>
